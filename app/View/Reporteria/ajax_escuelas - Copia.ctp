...
<div class="row-fluid">
	<div class="span10 offset1">	<br>				
			<div id="collapseReporte" class="accordion-body">
				<div class="accordion-inner" id="divEscuela">
					<!-- COMIENZA EL REPORTE -->
					<div class="row-fluid">
						<div class="span9">
								...
								<?php if ($totales) { ?>
									<table class="table table-striped table-bordered" id="tabla_datos">
										<tbody>
											<tr>
												<th>ESCUELA</th>
												<th><a href="javascript:orderEscuela('carrera');" >CARRERA <i class="icon-caret-down flechita"></i></a></th>
												<?php 
												if ($si_en_revision) { ?>
													<th><a href="javascript:orderEscuela('en_revision');" >EN REVISIÓN <i class="icon-caret-down flechita2"></i></a></th>
												<?php } ?>
												<?php 
												if ($si_habilitado_sin_firma) { ?>
													<th><a href="javascript:orderEscuela('habilitado_sin_firma');" >HABILITADAS <i class="icon-caret-down flechita2"></i></a></th>
												<?php } ?>
												<?php 
												if ($si_habilitado_firma) { ?>
													<th><a href="javascript:orderEscuela('habilitado_firma');" >HABILITADAS FIRMA <i class="icon-caret-down flechita2"></i></a></th>
												<?php } ?>
												<?php 
												if ($si_habilitado_csa) { ?>
													<th><a href="javascript:orderEscuela('habilitado_csa');" >HABILITADAS CSA <i class="icon-caret-down flechita2"></i></a></th>
												<?php } ?>
												<?php 
												if ($si_no_habilitado) { ?>
													<th><a href="javascript:orderEscuela('no_habilitado');" >NO HABILITADAS <i class="icon-caret-down flechita2"></i></a></th>
												<?php } ?>
													<th><a href="javascript:orderEscuela('total');" >TOTAL GENERAL <i class="icon-caret-down flechita2"></i></a></th>
											</tr>
											...
											foreach($totales as $key_escuela => $total) { 
											    ...
											    if(parseInt(suma)>0){
											    	...
											    	$('.title_<?php echo $key_carrera; ?>').hide();
											    	$('.<?php echo $key_carrera; ?>').hide();
											    	$('.total_<?php echo $key_carrera; ?>').hide();
											    }
									...
									<tbody>
										...
										<tr>
											...
											if(isset($escuelas_rs[$total['key_escuela']]) && $escuelas_rs[$total['key_escuela']]=='1'){
												echo '<strong>'.utf8_encode($total['nombre_escuela']).'</strong>'; 
											}else{
												echo utf8_encode($total['nombre_escuela']); 
											}
											...
											<?php 
											if(isset($escuelas_rs[$total['key_escuela']]) && $escuelas_rs[$total['key_escuela']]=='1'){
												echo '<strong>'.utf8_encode($total['en_revision']).'</strong>'; 
											}else{
												echo utf8_encode($total['en_revision']); 
											}
	...
	$('#torta1').highcharts({
				...
				series: [{
					...
					data: (function() {
								...
								{ ?>
								data.push(["<?php echo utf8_encode($totales_1[$key]['nombre_escuela']);?>",<?php echo $totales_1[$key]['en_revision'];?>]);
								<?php } ?>
								return data;
							})()
						}]
								...
								{  
									$cont_carreras = 0;
										foreach($total as $carreras) { 
											...
											data.push(["<?php echo utf8_encode($carreras['nombre_carrera']);?>",<?php echo $carreras['en_revision'] + $carreras['habilitado'] + $carreras['no_habilitado'];?>]);
