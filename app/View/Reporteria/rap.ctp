...
<div  class="row-fluid">
			<div class="span10 offset1">
				<h3>Reportes de RAP:</h3>
				<br>			
			</div>
</div>
<?php echo $this->Form->create('Rap', array('div' => false));?>
<div class="row-fluid">
						...
						<div id="collapseThree" class="accordion-body collapse">
						  <div class="accordion-inner">
							...
							<div class="row-fluid">
								<br>
								<div class="span2">
									<?php echo $this->Form->input('form_completado', array('value' => 0,'class' => 'opciones','label' => array('text' => 'FORMULARIO DE POSTULACION COMPLETADO'),'type' => 'checkbox','checked' => 'checked')); ?>
								</div>

								<div class="span2">
									<?php echo $this->Form->input('doc_recibida_revision', array('value' => 0,'class' => 'opciones','label' => array('text' => 'DOCUMENTACION RECIBIDA EN REVISION'),'type' => 'checkbox','checked' => 'checked')); ?>
								</div>

								<div class="span2">
									<?php echo $this->Form->input('cv_completado', array('value' => 0,'class' => 'opciones','label' => array('text' => 'CURRICULUM RAP COMPLETADO'),'type' => 'checkbox','checked' => 'checked')); ?>
								</div>

								<div class="span2">
									<?php echo $this->Form->input('doc_aprobada', array('value' => 0,'class' => 'opciones','label' => array('text' => 'DOCUMENTACION APROBADA'),'type' => 'checkbox','checked' => 'checked')); ?>
								</div>

								<div class="span2">
									<?php echo $this->Form->input('cv_revision', array('value' => 0,'class' => 'opciones','label' => array('text' => 'CV RAP Y AUTOEVALUACION COMPLETADA EN REVISION'),'type' => 'checkbox','checked' => 'checked')); ?>
								</div>

								<div class="span2">
									<?php echo $this->Form->input('cv_aprobada', array('value' => 0,'class' => 'opciones','label' => array('text' => 'CV RAP Y AUTOEVALUACION APROBADA'),'type' => 'checkbox','checked' => 'checked')); ?>
								</div>

								<div class="span2">
									<?php echo $this->Form->input('entrevista', array('value' => 0,'class' => 'opciones','label' => array('text' => 'ENTREVISTA AGENDADA'),'type' => 'checkbox','checked' => 'checked')); ?>
								</div>
...
{
		$('#RapFechaDesde').val('01/03/2014');
		var hoy = new Date();
		var dia = hoy.getDate();
		var mes = hoy.getMonth() + 1;
		var ano = hoy.getFullYear();		
		fecha = dia+'/'+mes+'/'+ano; 
		$('#RapFechaHasta').val(fecha);
		validarFechas();
}

function validarFechas()
{
	var formulario = $('#RapRapForm').serialize();
	var desde		= $('#RapFechaDesde').val();
	var hasta		= $('#RapFechaHasta').val();
	...
	if(timestamp_desde >= timestamp_hasta)
	{
		...
		if($('#RapFechaDesde').val() == '' || $('#RapFechaHasta').val() == '' )
		{
		...
		$.ajax({
				url: webroot +'reporteria/ajax_rap',
				type: 'POST',
				async: true,
				data: formulario,
...
<script>
	$('#RapFechaDesde').hover(function() {
	$(this).css('cursor','pointer');
	});
	$('#RapFechaHasta').hover(function() {
	$(this).css('cursor','pointer');
	});
...
///para exportar a excel
	$('.excel').click(function(){
		window.location.href =  webroot +'reporteria/ajax_exportar_excel_rap';
	})
