<?php
	...
	class AdministrativosController extends AppController {
		...
		public function ajax_autocompletar(){	
			...
			$term 	   = isset($this->params['pass'][1]) ? $this->params['pass'][1] : '';
			...
			if ($tipo == 'PREPOSTULACIONES'){ //BUSCAMOS POR PREPOSTULACIONES	
				$result = array();
				$users = $this->Prepostulacion->find('all', array('conditions' =>  array('Prepostulacion.guardado' => '1')));

				foreach($users as $key => $user){
					$user['Postulante']['apellidom'] = ($user['Postulante']['apellidom']);								
					$user['Postulante']['apellidop'] = ($user['Postulante']['apellidop']);
					$user['Postulante']['nombre']    = ($user['Postulante']['nombre']);
					...
					$rs_cadena = trim($nombre).' '.trim($apellidop).' '.trim($apellidom).', '.trim($email).', '. trim($rut);
					
					//$html .= '<option value="'.$rut.'">'.$rs_cadena.'</option>';
					if($nombre!='' && $rut!=''){
						array_push($result, htmlentities(utf8_decode($rs_cadena))); 
						array_push($rs_rut, $rut);
					}
			...
			}elseif($tipo == 'PREPOSTULACIONESID'){
				//$Id = $this->request->data['id'];
				$result = array();
				$html = '<option value=""></option>';
				$users = $this->Prepostulacion->find('all', array('conditions' =>  array('Prepostulacion.id_correlativo LIKE' => '%AEE%', 'Prepostulacion.guardado' => '1')));
				...
				foreach($users as $key => $user){
					...
					$email 			= $user['Postulante']['email']!='' 		&& $user['Postulante']['email']!='null' 	? $user['Postulante']['email'] : '';
					$rut 			= $user['Postulante']['rut']!='' 		&& $user['Postulante']['rut']!='null' 	? $user['Postulante']['rut'] : '';
					$id_correlativo = $user['Prepostulacion']['id_correlativo']!='' && $user['Prepostulacion']['id_correlativo']!='null' ? $user['Prepostulacion']['id_correlativo'] : '';
					$codigo 		= $user['Prepostulacion']['codigo']!='' && $user['Prepostulacion']['codigo']!='null' ? $user['Prepostulacion']['codigo'] : '';
					
					$rs_cadena = $id_correlativo.', '.$nombre.' '.$apellidop.' '.$apellidom.', '.$email.', '. $rut;

					//$html .= '<option value="'.$codigo.'">'.$rs_cadena.'</option>';
					if($nombre!='' && $rut!=''){
						array_push($result, htmlentities(utf8_decode($rs_cadena))); 
						array_push($rs_rut, $codigo);
					}
				} 
				//var_dump($html); die;
				//$result = $html;
			}elseif ($tipo == 'POSTULANTE'){ //BUSCAMOS POR PREPOSTULACIONES	
				$result = array();
				$users = $this->Postulante->find('all');
				$html = '<option value=""></option>';
				foreach($users as $key => $user){
					$user['Postulante']['apellidom'] = ($user['Postulante']['apellidom']);										
					$user['Postulante']['apellidop'] = ($user['Postulante']['apellidop']);
			...
			elseif ($tipo == 'TODOSID'){
				$result = array();
				$html = '<option value=""></option>';
				$condicion =  array('Postulacion.id_correlativo LIKE' => '%AEE%','Postulacion.tipo <>' => 'RAP');
				...
				foreach($users as $key => $user){
					...
					$email = $user['Postulante']['email']!='' && $user['Postulante']['email']!='null' ? $user['Postulante']['email'] : '';
					$rut = $user['Postulante']['rut']!='' && $user['Postulante']['rut']!='null' ? $user['Postulante']['rut'] : '';
					$id_correlativo = $user['Postulacion']['id_correlativo']!='' && $user['Postulacion']['id_correlativo']!='null' ? $user['Postulacion']['id_correlativo'] : '';
					$codigo = $user['Postulacion']['codigo']!='' && $user['Postulacion']['codigo']!='null' ? $user['Postulacion']['codigo'] : '';
					
					$rs_cadena = $id_correlativo.', '.$nombre.' '.$apellidop.' '.$apellidom.', '.$email.', '. $rut;

					if($nombre!='null' && $codigo!=''){
						array_push($result, htmlentities(utf8_decode($rs_cadena)));
						array_push($rs_rut, $codigo);
					}
				}
				...
				$option = array(
					...
					'fields' => array('Postulante.nombre','Postulante.apellidop','Postulante.apellidom','Postulante.email','Postulante.rut'),
					'group' => array('Postulante.nombre','Postulante.apellidop','Postulante.apellidom','Postulante.email','Postulante.rut')
				);
				$users = $this->Postulacion->find('all', $option);
				//var_dump($users); die;
				foreach($users as $key => $user){
					$user['Postulante']['apellidom'] = ($user['Postulante']['apellidom']);
					$user['Postulante']['apellidop'] = ($user['Postulante']['apellidop']);
			...
			elseif ($tipo == 'RAPTODOSID'){
				...
				foreach($users as $key => $user){
					...
					$rs_cadena = trim($id_correlativo).', '.trim($nombre).' '.trim($apellidop).' '.trim($apellidom).', '.trim($email).', '.trim($rut);
			...
			//$users = $result;
			echo json_encode(array("result" => array_unique($result), "value" => array_unique($rs_rut)));			
		}
				...
				switch ($tipo) {
					...
					//echo var_dump($tipo).'3'; die;					
						...
						if (!empty($postulantes)){
								...
								$k =0;
								if(is_array($postulaciones) && count($postulaciones)>0){
									foreach ($postulaciones as  $value) {
										$estado_postulaciones = $this->EstadoPostulacion->find('all', array('conditions' => array('EstadoPostulacion.postulacion_codigo' => $value['Prepostulacion']['codigo_postulacion'])) );
										...
										if(count($estado_postulaciones)>0){
											...
											$postulaciones[$k]['EstadoPostulacion'] = $array_estados;
										}
...
/* 				$licencia_educacion_media = array();
		...
		function listadopostulaciones($buscar_estado = null){
			$this->response->disableCache();
...
/*													   										 				  
	...
	public function listadoPostulacionesNuevasClose(){
	...
	public function responderPostulante(){
		...
		if (($respuesta['Administrativo']['habilitar'] == null) || ($respuesta['Administrativo']['codigo_postulacion'] == null)) {
			...
			$this->redirect(array('action' => 'listadoPostulacionesNuevasClose'));				
		}
