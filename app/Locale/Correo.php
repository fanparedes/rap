...
class Correo extends AppModel {
	...
	public function enviarEmail($postulante, $tipo, $plazo = null, $plazo2 = null, $carreras = null){	
			...
			switch ($tipo) {
				...
				case 2: //Acepta la documentación del postulante RAP.
					...
					$Email->viewVars(array('postulante'=>$postulante,'plazo1' => $plazo, 'plazo2' => $plazo2));	
					$tipo_mail = 2;
					break;
				...
				case 6: //ENVIO AL POSTULANTE CUANDO REALIZA UNA PREPOSTULACIÓN										
					...
					$Email->subject('[Portal – Admisión Especial]  Solicitud de habilitación generada');			    
				    $Email->viewVars(array('postulante'=>$postulante));
					$tipo_mail = 6;
					break;
				case 7: 
					...
					foreach ($administrativos as $administrativo){
							...
							$Email->subject('[Portal – Admisión Especial]  Se ha creado una nueva postulación');				    
							$Email->viewVars(array('postulante'=>$postulante));
							$Email->send();
					}
					return true;
				...
				case 8: // Se avisa al postulante que la documentación ha sido invalidada.
					$Email->subject('[Portal – Admisión Especial] Documentos con observaciones');
					$Email->template('documentosinvalidos', 'postulante');
					$Email->viewVars(array('postulante'=>$postulante));
				break;
				case 9: // Se avisa al postulante que la documentación ha sido validada.
					$Email->subject('[Portal – Admisión Especial] Documentación aprobada. Debe completar el formulario de postulación');
				...
				case 10: // Se avisa al coordinador que se ha habilitado un postulante por parte de un administrativo.
					...
					foreach ($administrativos as $administrativo){
							...
							$Email->subject('[Portal – Admisión Especial]  Se ha habilitado a un postulante');				    
							$Email->viewVars(array('postulante'=>$postulante));
							$Email->send();
							return true;
					}
				...
				case 11: // Se avisa al coordinador general que se ha 
					...
					foreach ($administrativos as $administrativo){
							...
							$Email->subject('[Portal – Admisión Especial]  Se ha inhabilitado a un postulante');				    
							$Email->viewVars(array('postulante'=>$postulante));
							$Email->send();
							return true;
					}
				...
				case 12: // Se avisa al administrativo RAP que una persona ha accedido al sistema RAP
					...
					foreach ($administrativos as $administrativo){
							if ($administrativo['Administrativo']['email'] !== null){
								...
								$Email->subject('[Portal – Admisión Especial]  Nueva Solicitud de habilitación RAP');				    
								$Email->viewVars(array('postulante'=>$postulante));
								if ($Email->send()){
									return true;
								}
				...
				case 13: // Se avisa al administrativo AV que una persona ha accedido al sistema aV
					...
					foreach ($administrativos as $administrativo){		
							if ($administrativo['Administrativo']['email'] !== null){								
								...
								$Email->subject('[Portal – Admisión Especial]  Nueva Solicitud de habilitación');				    
								$Email->viewVars(array('postulante'=>$postulante));	
								if ($Email->send() == true){
				...
				case 14: // Se avisa al administrativo AH que una persona ha accedido al sistema ESCUELAS	
					...
					foreach ($administrativos as $k => $administrativo){									
								...
								$k->subject('[Portal - Admision Especial] Nueva Solicitud de habilitación en su ESCUELA');
					...
					case 15: //ENVIO AL POSTULANTE CUANDO REALIZA UNA POSTULACIÓN										
						...
						$Email->subject('[Portal – Admisión Especial]  Postulación Modificada');			    
						$Email->viewVars(array('postulante'=>$postulante));
					break;
					case 16: //Se envia un email a cada administrativo de la misma carrera					
						...
						$administrativos2 .= ')';
						$condicion = array(('Postulacion.carrera_codigo IN '.$administrativos2.''));
						echo var_dump($condicion);
						$EscuelaCarrera = ClassRegistry::init('EscuelaCarrera');	
						$Administrativo = ClassRegistry::init('Administrativo');	
						$administrativos = $Administrativo->find('all', array('conditions' => $condicion));
						echo var_dump($administrativos);
						foreach ($administrativos as $k => $administrativo){									
								$k = '';									
								$k = new CakeEmail('smtp');
